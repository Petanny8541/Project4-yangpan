    ...
    namespace interiorlighting
    {
        namespace interactor
        {
            public class SceneInteractor : MonoBehaviour
            {
                ...
                {
                    var lightIntensity = GameObject.FindGameObjectsWithTag("DynamicLights");

                    float intensity = go.GetComponent<LightingIntensityValue>().intensity;

                    for (int i = 0; i < lightIntensity.Length; i++)
                    {
                        lightIntensity[i].GetComponent<Light>().intensity = intensity;
                    }
                ...
                {
                    var lightColor = GameObject.FindGameObjectsWithTag("DynamicLights");

                    for (int i = 0; i < lightColor.Length; i++)
                    {
                        lightColor[i].GetComponent<Light>().color = c;
                    }
                ...
                void OnTriggerEnter(Collider c)
                {
                    ...
                    else if (gameObjectThatWasHit == "BlueCube")
                    {
                    ...
                    else if (gameObjectThatWasHit == "IntensityCube0.0" || gameObjectThatWasHit == "IntensityCube0.25" || gameObjectThatWasHit == "IntensityCube0.5"
                        || gameObjectThatWasHit == "IntensityCube0.75" || gameObjectThatWasHit == "IntensityCube1.0")
                    {
                        SetLightingIntensity(c.gameObject);
                    }
                    else if (gameObjectThatWasHit == "DoorRotationAxis")
                    {
